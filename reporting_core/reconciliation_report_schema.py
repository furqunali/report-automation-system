...
def validate_reconciliation_report(payload: dict) -> bool:
    if not isinstance(payload, dict) or set(payload) != REQUIRED_FIELDS:
        return False
    if not all(isinstance(payload[name], int) and payload[name] >= 0 for name in ("count","balanced","unbalanced")):
        return False
    ratio = payload["balanced_ratio"]
    return isinstance(ratio, (int, float)) and 0 <= ratio <= 1 and payload["balanced"] + payload["unbalanced"] == payload["count"]
